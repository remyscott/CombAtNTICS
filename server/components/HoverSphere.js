...
export class HoverSphere {
  constructor(player) {
    const defaultOpts = { radius: 0.5, force: 10, density: 1, friction: 0.5, restitution: 0.5 };
    this.opts = defaultOpts;

    this.body = player.world.createBody({
      type: "dynamic",
      position: {x:1, y:1},
      userData: {id: player.world.newBodyId(), owner: player, type: 'player', scale: this.opts.radius*2}
    });
